...
export class DetailProductComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.styledLog('Nguồn này không có gì để xem ', 'color: red ; font-size: 22px;');
        this.styledLog('Đây là một tính năng của trình duyệt dành cho các nhà phát triển. Nếu ai đó bảo bạn sao chép-dán nội dung nào đó vào đây để bật một tính năng , bạn nên dừng lại!', 'color: yellow  ; font-size: 15px;');
        debugger
        ...
        if(!isNaN(this.productId)){
            ...
            this.productService.getDetailProduct(this.productId).subscribe({
                next : (response : any) => {
                    ...
                    if (response.product_images && typeof response.product_images === 'string') {
                        // Chuyển đổi chuỗi JSON thành đối tượng JavaScript
                        response.product_images = JSON.parse(response.product_images);
                      }
                  //ktra đổi Json sang array
                    if(Array.isArray(response.product_images) && response.product_images.length > 0 ){
                        response.product_images.forEach((product_images:ProductImage) =>
                        {
                            debugger
                            product_images.imageUrl = environment.apiBaseUrl+'/products/image/'+product_images.imageUrl
                        })
                    }
                    debugger
